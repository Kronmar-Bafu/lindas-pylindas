...
from rdflib import Graph, URIRef, SH
...
def validation_job(args):
    target, shape = args
    r = validate(target, shape)

if __name__ == "__main__":
    times = []
    num_observations = [1e2, 1e3, 1e4, 1e5]
    num_dimensions = [1,10]
    ...
    for num_obs in num_observations:
        for num_dim in num_dimensions:
            ...
            shape = Graph().parse("benchmarking/0.ttl")
            shape.add((URIRef("https://benchmark.ld.admin.ch/cube/benchmark_1/1/shape"), SH.targetClass, URIRef("https://cube.link/Observation")))

            validation_args = [(Graph().parse(f"benchmarking/{i}.ttl"), shape) for i in range(num_workers)]

            with Pool(processes=num_workers) as pool:
                pool.map(validation_job, validation_args)

            print(f"Time for {num_obs} observations with {num_dim} dimensions and {num_workers} workers with validation: {time() - tic}")
            times.append(time() - tic)
    ...
    df.to_csv("benchmarking/results_with_validation_multiprocessing.csv", index=False)
